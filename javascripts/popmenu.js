...
var PopMenu = function createPopMenu(){
	...
	var thumb; // thumb that triggers the right-click menu
	var thumbImg;
	popMenu.style.display = "none";
	for (i = 0; i < aLi.length; i++){
		// Add arrow for li with sub-menu
	...
	aLi[1].onmousedown = function(event){ // Face-up Attack Position
	...
	aLi[2].onmousedown = function(event){ // Face-up Defense Position
		var tmplItem = $(thumb).tmplItem().data;
		var card_id = tmplItem.card_info.card_id;
		thumbImg.src = card_img_thumb_url + card_id + ".jpg";
	...
	aLi[3].onmousedown = function(event){ // Face-down Defense Position
		var tmplItem = $(thumb).tmplItem().data;
		var card_id = tmplItem.card_info.card_id;
	...
	aLi[4].onmousedown = function(event){ // Face-down Attack Position
		var tmplItem = $(thumb).tmplItem().data;
		var card_id = tmplItem.card_info.card_id;
		thumbImg.src = "images/unknow.jpg";
	...
	aLi[5].onmousedown = function(event){ // Disable Revive Limit
		var tmplItem = $(thumb).tmplItem().data;
		tmplItem.card_info.disable_revivelimit = false;
	}
	aLi[6].onmousedown = function(event){ // Enable Revive Limit
		var tmplItem = $(thumb).tmplItem().data;
		tmplItem.card_info.disable_revivelimit = true;
	}
	aLi[7].onmousedown = function(event){ // Add Continuous Effect Target
		...
		if(!selectingContinuous){
			alert("No suitable continuous effect target on the field!");
		}
	}
	aLi[8].onmousedown = function(event){ // Remove Continuous Effect Target
		thumb_continuous = thumb;
	...
	aLi[9].onmousedown = function(event){ // Set Equip Target
		thumb_equip = thumb;
		var fields = GetAllFields();
		for(var i=0; i< fields.length;i++){
			var tmplItem = $(fields[i]).tmplItem().data;
		...
		if(!selectingEquip){
			alert("No suitable equip target on the field!");
		}
	}
	aLi[10].onmousedown = function(event){ // Remove Equip Target
		var card_info = $(thumb).tmplItem().data.card_info;
		var equip_target = card_info.equip_target[0];
		removeEquipRelation(thumb, equip_target);
		removeBeEquipRelation(equip_target, thumb);
	}
	aLi[11].onmousedown = function(event){ // Place Counter
		var tmplItem = $(thumb).tmplItem().data;
		...
		if(card_counters.length == 0){	
			addCounterSelector();// Add default counter selector
		}
		else {
			for(var i=0; i<card_counters.length; i++){// Set selected items and quantity in counter selectors
	...
	aLi[14].onmousedown = function(event){ // Set Attack Points
		var card_info = $(thumb).tmplItem().data.card_info;
		var attack = prompt("Enter Attack Points!", card_info.attack || 0);
		if(attack != undefined && !isNaN(attack)){
			card_info.attack = attack;
		}
	}
	aLi[15].onmousedown = function(event){ // Set Base Attack Points
		var card_info = $(thumb).tmplItem().data.card_info;
		var base_attack = prompt("Enter Base Attack Points!", card_info.base_attack || 0);
		if(base_attack != undefined && !isNaN(base_attack)){
			card_info.base_attack = base_attack;
		}
	}
	aLi[17].onmousedown = function(event){ // Set Defense Points
		var card_info = $(thumb).tmplItem().data.card_info;
		var defence = prompt("Enter Defense Points!", card_info.defence || 0);
		if(defence != undefined && !isNaN(defence)){
			card_info.defence = defence;
		}
	}
	aLi[18].onmousedown = function(event){ // Set Base Defense Points
		var card_info = $(thumb).tmplItem().data.card_info;
		var base_defence = prompt("Enter Base Defense Points!", card_info.base_defence || 0);
		if(base_defence != undefined && !isNaN(base_defence)){
			card_info.base_defence = base_defence;
		}
	}
	aLi[19].onmousedown = function(event){ // Set Level
		var card_info = $(thumb).tmplItem().data.card_info;
		var level = prompt("Enter Level!", card_info.level || 0);
		if(level != undefined && !isNaN(level)){
			card_info.level = level;
		}
	}
	aLi[20].onmousedown = function(event){ // Adjust Order
...
function delCounterSelector(ev){ // Remove counter selector on the same row
	ev         = ev || window.event;
...
function addCounterSelector(code, number){ // Add counter selector
	$("#CounterSelector-tmpl").tmpl({
		counters: counters,
		_code: code || "0x3001",
		_number: number || 0
	}).appendTo($('#counterSelectors'));
	$('.spinner').spinner({ // Set counter quantity cannot be less than 1
...
function addCounter(dialog, thumb){ // Update thumb with the content of dialog
	var card_info = $(thumb).tmplItem().data.card_info;
...
function sort(dialog, field){ // Update field based on the order in dialog
